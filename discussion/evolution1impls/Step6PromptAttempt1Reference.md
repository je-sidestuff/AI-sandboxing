...
I've completed the implementation of the `heuristic-agent` sub-project. Let me provide a summary of what was created:

## Summary: `research/AI-evo1/heuristic-agent`
...
**1. Two Entrypoints (Single Binary with Subcommands)**
- `heuristic-agent watch [--agent-type <type>]` - Watch loop for processing work signals
- `heuristic-agent slopspace <create|deploy|return|list|delete|status>` - Slopspace management

**2. Slopspace Lifecycle**
- Creation in `/host-agent-files/slopspaces/<id>/` with read-spaces and write-spaces
- Deployment moves content to `/agent/<agent-type>/`
- Return moves write-spaces back, discards read-spaces
- Metadata and sensitive files remain in slopspaces directory

**3. Work Signal Management**
- JSONL format: `WORKING-<name>-<timestamp>.jsonl` / `COMPLETE-<name>-<timestamp>.jsonl`
- Header contains work configuration (agent type, role, prompt, status, holder)
- Events appended as additional JSON lines
- Ownership/holder tracking for distributed coordination

**4. Environment Variables**
- `SLOPSPACES_DIR` (default: `/host-agent-files/slopspaces`)
- `WORK_SIGNALS_DIR` (default: `/host-agent-files/work`)
- `AGENT_SLOPSPACE_ROOT` (default: `/agent`)
- `AGENT_RECORDS_PATH` (default: `/host-agent-files/agent-records`)

**5. Clauditable Integration**
- All system calls wrapped with clauditable for recording
- Detects if already in clauditable context to avoid double-wrapping
- Uses ambiguous-agent for AI agent invocation

**6. Bootstrapping Files**
- Makefile with standard targets (build, test, clean, deploy-dependencies-local)
- Dockerfile that includes all monorepo runtime dependencies
- GitHub Actions workflow for CI testing on `test/heuristic-agent` branch

### Updated Root Files
- `research/AI-evo1/Makefile` - Added heuristic-agent to SUBPROJECTS
- `research/AI-evo1/Dockerfile` - Added heuristic-agent build and copy steps
- `research/AI-evo1/.github/workflows/heuristic-agent-test.yml` - CI workflow

Records saved to: /workspaces/agent-records/session-2026-04-29_00-47-14_1777423634/2026-05-02_18-36-00_1777746960
