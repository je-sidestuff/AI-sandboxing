...
The AI has responded to the first prompt in this step.
...
To RETRY from a previous point replace the '<REPLACE-Revision|Retry>' with 'Retry'. By default this retries from the previous increment. To retry from further back add '(from start)' or '(from X)' where X is a revision letter, for example '## Retry A (from start)'.
