...
# Description: This script sets up a to-repo containment environment for AI processing.
#              It clones a source repository, pushes the contents to a newly created
#              target repository, separates the .git state for security, and places
#              the work unit in the SLOPSPACES input directory for the agent-worker
#              to process.
#
# Required environment variables:
#   UNIX_TIMESTAMP      - Unix seconds timestamp for unique directory naming
#   BRANCH_NAME         - The dispatch branch name to create in the target repo
#   SLOPSPACES_WORK_DIR - The SLOPSPACES working directory (e.g., /workspaces/slopspaces/working/)
#   SOURCE_REPO_URL     - The authenticated HTTPS URL of the source repository to clone from
#   TARGET_REPO_URL     - The authenticated HTTPS URL of the newly created target repository
...
#     - INSTRUCTION.json  (instructions for the agent)
...
echo "========================================"
echo "To-Repo Containment Setup"
echo "========================================"
...
echo "Branch name: ${BRANCH_NAME} (will be created in target repo)"
echo "========================================"
...
# Step 2: Clone the source repo for baseline content
echo ""
echo "Step 2: Cloning source repo for baseline content..."
...
if [ $? -ne 0 ]; then
    clean_up_and_report_failure "Failed to clone source repository"
fi
echo "Cloned source repository to: ${OUTER_DIR}/repo"

# Step 3: Push baseline to target repo and create the dispatch branch
echo ""
echo "Step 3: Pushing baseline to target repo and creating branch ${BRANCH_NAME}..."
...
echo "Pushed baseline to target repo and created branch: ${BRANCH_NAME}"
