...
# Move our git state out of the way for a moment
mv "$TMP_DIR/repo/.git" "$TMP_DIR/git_state"

# Move our folder to /workspaces/slopspaces so that the AI can find it and work with it
mv "$TMP_DIR/repo" "/workspaces/slopspaces/input_queue/repo"

# Wait for up to 10 minutes for the AI to put the completed work in slopspaces/processed
echo "Waiting for the AI to process the repository and place the result in /workspaces/slopspaces/processed/repo..."
TIMEOUT=600 # 10 minutes in seconds
ELAPSED=0
while [ ! -d "/workspaces/slopspaces/processed/repo" ]; do
    sleep 5
    ELAPSED=$((ELAPSED + 5))
...
mv "/workspaces/slopspaces/processed/repo" "$TMP_DIR/processed-repo"

# Bring the git state back and push to our branch
mv "$TMP_DIR/git_state" "$TMP_DIR/processed-repo/.git"
